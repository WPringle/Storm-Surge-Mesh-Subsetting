...
plot(m,'type','blog','proj','merc','backcolor',[0.7 0.7 0.7],'fontsize',10);
title('')
m_plot(track(:,1),track(:,2),'r-','linew',tw);
m_plot(swath_vec(:,1),swath_vec(:,2),'r--','linew',tw);
print([direc coarse '_blog.png'],figres,'-dpng')

%% Plot coarse mesh zoom-in
plot(m,'type','bmesh','proj','lam','subdomain',bou_buff,...
     'colormap',[22 -10 100],'backcolor',[0.7 0.7 0.7],'fontsize',10);
title('')
m_plot(track(:,1),track(:,2),'r-','linew',tw);
m_plot(swath_vec(:,1),swath_vec(:,2),'r--','linew',tw);
print([direc coarse '_bmesh.png'],figres,'-dpng')

%% Plot fine mesh zoom-in
load([fine '.mat'])
plot(m,'type','bmesh','proj','lam','subdomain',bou_buff,...
    'colormap',[22 -10 100],'backcolor',[0.7 0.7 0.7],'fontsize',10);
title('')
m_plot(track(:,1),track(:,2),'r-','linew',tw);
m_plot(swath_vec(:,1),swath_vec(:,2),'r--','linew',tw);
print([direc fine '_bmesh.png'],figres,'-dpng')
