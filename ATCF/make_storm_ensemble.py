...
def main(number_of_perturbations, variable_list, storm_code, start_date,
         end_date):
    # Example:
    ...
    # getting best track
    BT = BestTrackForcing(storm_code, start_date=start_date,
                          end_date=end_date)
    ...
    for var in variable_list:
        ...
        for idx in range(1, number_of_perturbations + 1):
            ...
            if var == vmax_var:
                ...
                df_modified[pc_var] = compute_pc_from_Vmax(df_modified,
                                                           Holland_B)
            # reset the dataframe
            BT._df = df_modified
...
def interpolate_along_track(df_, VT, along_track_errors):
    ...
    for idx in range(1, len(track_coords) - 1):
        ...
        # get back lat-lon
        lon, lat = myProj(pnew.coords[0][0], pnew.coords[0][1],
                          inverse=True)
...
def offset_track(df_, VT, cross_track_errors):
    ...
    for idx in range(0, len(track_coords)):
        ...
        lon, lat = myProj(x_ref + alpha * dx, y_ref + alpha * dy,
                          inverse=True)
        lon_new.append(lon)
        lat_new.append(lat)
